...
# frame = cv2.imread("saved_frame.jpg")  # Use a saved image for testing
    ...
    if results.masks is not None and len(results.masks.data) > 0:
        ...
        # 2. Compute the best-fit rotated rectangle
        rect = cv2.minAreaRect(points)  # (center, (width, height), angle)

        # 3. Get rectangle corner points
        box = cv2.boxPoints(rect)
        box = np.int0(box)  # convert to integer

        # 4. Draw the rectangle on a copy of the image
        vis = cv2.cvtColor(adaptive_clean, cv2.COLOR_GRAY2BGR)
        cv2.drawContours(vis, [box], 0, (0, 255, 0), 2)

        cv2.imshow("Best Fit Rotated Rectangle", vis)
